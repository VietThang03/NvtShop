...
import CreateEditCity from './CreateEditProductType'
import { OBJECT_TYPE_ERROR_DELIVERY, OBJECT_TYPE_ERROR_PRODUCT_TYPE } from 'src/configs/error'
import { formatDate } from 'src/utils/date'
import CreateEditDeliveryType from './CreateEditProductType'
import { deleteMultipleProductTypeAsync, deleteProductTypeAsync, getAllProductTypesAsync } from 'src/stores/apps/product-type/actions'
import { resetInitialState } from 'src/stores/apps/product-type'
import CreateEditProductType from './CreateEditProductType'
...
const ProductTypeListPage = () => {
  ...
  const [openDeleteMultipleProductType, setOpenDeleteMultipleProductType] = useState(false)
  const [sortBy, setSortBy] = useState<string>('created desc')
  const [searchBy, setSearchBy] = useState<string>('')
  const [selectedRow, setSelectedRow] = useState<string[]>([])
  const [loading, setLoading] = useState<boolean>(false)
  //permission
  const { CREATE, DELETE, UPDATE, VIEW } = usePermission('MANAGE_PRODUCT.PRODUCT_TYPE', [
    'CREATE',
    'VIEW',
    'UPDATE',
    'DELETE'
  ...
  const handleGetListProductType = () => {
    dispatch(getAllProductTypesAsync({ params: { limit: pageSize, page: page, search: searchBy, order: sortBy } }))
  }
  ...
  const handleCloseConfirmDeleteMultipleProductType = useCallback(() => {
    setOpenDeleteMultipleProductType(false)
  }, [])
  ...
  const handleAction = (action: string) => {
    switch (action) {
      case 'delete': {
        setOpenDeleteMultipleProductType(true)
        break
      }
    }
  }
  ...
  return (
    <>
      ...
      />
      <ConfirmationDialog
        open={openDeleteMultipleProductType}
