...
export const getAllPaymentTypes = async (data: { params: TParamsGetPaymentTypes }) => {
  try {
    const res = await instanceAxios.get(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}`, data)
...
export const createPaymentType = async (data: TParamsCreatePaymentType) => {
  try {
    const res = await instanceAxios.post(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}`, data)
    console.log(res)
    return res.data
  } catch (error: any) {
...
export const updatePaymentType = async (data: TParamsEditPaymentType) => {
  const { id, ...rests } = data
  try {
    const res = await instanceAxios.put(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}/${id}`, rests)
...
export const deletePaymentType = async (id: string) => {
  try {
    const res = await instanceAxios.delete(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}/${id}`)
...
export const getDetailsPaymentType = async (id: string) => {
  try {
    const res = await instanceAxios.get(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}/${id}`)
...
export const deleteMultiplePaymentType = async (data: TParamsDeleteMultiplePaymentType) => {
  try {
    const res = await instanceAxios.delete(`${CONFIG_API.SETTING.PAYMENT_TYPE.INDEX}/delete-many`, { data })
